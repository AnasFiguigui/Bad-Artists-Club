...
io.on('connection', (socket) => {
    ...
    socket.on('draw', (data) => {
        if (!rateLimit(socket.id, 'draw', 60, 1000))
            return;
    ...
    socket.on('request-game-state', (data, callback) => {
    ...
    socket.on('reroll', (data, callback) => {
    ...
    socket.on('skip-turn', (data, callback) => {
    ...
    socket.on('clear-canvas', (data) => {
        if (!rateLimit(socket.id, 'clear-canvas', 5, 5000))
            return;
        try {
    ...
    socket.on('undo', (data) => {
        if (!rateLimit(socket.id, 'undo', 10, 5000))
            return;
        try {
