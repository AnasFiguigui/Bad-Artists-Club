...
describe('GameManager Socket Events', () => {
  ...
  describe('Room Creation and Joining', () => {
    // @ts-ignore
    it('should emit room-created event when player creates room', (done: any) => {
      const mockSocket = { id: 'socket1', join: jest.fn(), emit: jest.fn() } as any

      // Mock the socket.io broadcast
      let broadcastedRoom: any = null
    ...
    // @ts-ignore
    it('should emit room-updated event when player joins', (done: any) => {
      const hostSocket = { id: 'host1', join: jest.fn() } as any
      const joinSocket = { id: 'join1', join: jest.fn(), emit: jest.fn(), to: jest.fn() } as any
      ...
      let joinRoomUpdated: any = null

      joinSocket.emit = jest.fn((event: string, data: any) => {
        if (event === 'room-updated') {
          joinRoomUpdated = data
        }
      })

      io.to = jest.fn((rid: string) => ({
        emit: (event: string, data: any) => {
  ...
  describe('Game Flow', () => {
    // @ts-ignore
    it('should emit game-started and round-start when host starts game', (done: any) => {
      const hostSocket = { id: 'host1', join: jest.fn() } as any
      ...
      // Mock event emission
      let gameStarted: any = null
      let roundStarted: any = null
    ...
    // @ts-ignore
    it('should verify ready event updates player status', (done: any) => {
