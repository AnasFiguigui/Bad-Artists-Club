...
export class GameManager {
    constructor(io, roomManager) {
        ...
        Object.defineProperty(this, "roundStartTimes", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        ...
        Object.defineProperty(this, "canvasStrokes", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    handleCreateRoom(socket, data) {
        ...
        if (!validThemes.includes(data.config.theme))
            throw new Error('Invalid theme');
        if (typeof data.config.rounds !== 'number' || data.config.rounds < 1 || data.config.rounds > 10)
    ...
    handleRequestGameState(socket, roomId) {
        const room = this.roomManager.getRoom(roomId);
        if (!room)
            return null;
    ...
    handleDraw(socket, stroke) {
        // Validate DrawStroke input
        if (!stroke || typeof stroke !== 'object')
            return;
        if (typeof stroke.roomId !== 'string')
            return;
        if (!Array.isArray(stroke.points) || stroke.points.length > 5000)
            return;
        if (typeof stroke.size !== 'number' || stroke.size < 1 || stroke.size > 100)
            return;
        if (typeof stroke.color !== 'string' || stroke.color.length > 20)
            return;
        if (!['brush', 'eraser', 'fill'].includes(stroke.tool))
            return;
        ...
        if (!room?.drawer || room.drawer !== socket.id)
            return;
    ...
    handleChatMessage(socket, roomId, message) {
        ...
        if (isCorrect) {
            ...
            const nonDrawerPlayers = room.players.filter((p) => p.id !== room.drawer);
    ...
    handleReroll(socket, roomId) {
        const room = this.roomManager.getRoom(roomId);
        if (!room)
            throw new Error('Room not found');
        if (room.drawer !== socket.id)
            throw new Error('Only the drawer can reroll');
        const character = this.selectRandomCharacter(room.theme);
        const hint = this.generateHint(character.name);
        this.roomManager.setAnswer(roomId, character.name, hint);
    ...
    handleSkipTurn(socket, roomId) {
        const room = this.roomManager.getRoom(roomId);
        if (!room)
            throw new Error('Room not found');
        if (room.drawer !== socket.id)
            throw new Error('Only the drawer can skip');
    ...
    handleUpdateSettings(socket, roomId, settings) {
        const room = this.roomManager.getRoom(roomId);
        if (!room)
            throw new Error('Room not found');
        if (room.host !== socket.id)
    ...
    handleKickPlayer(socket, roomId, targetId) {
        const room = this.roomManager.getRoom(roomId);
        if (!room)
            throw new Error('Room not found');
        if (room.host !== socket.id)
    ...
    handleClearCanvas(socket, roomId) {
        ...
        if (!room)
            throw new Error('Room not found');
        if (room.state !== 'results' && room.drawer !== socket.id)
            throw new Error('Only the drawer can clear');
    ...
    handleUndo(socket, roomId) {
        ...
        if (!room)
            throw new Error('Room not found');
        if (room.state !== 'results' && room.drawer !== socket.id)
            throw new Error('Only the drawer can undo');
    ...
    cleanupAfterLeave(room, leftSocketId) {
